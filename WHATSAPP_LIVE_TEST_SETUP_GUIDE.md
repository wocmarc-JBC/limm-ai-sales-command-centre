...
4. Copy the app's App Secret into the server-only `WHATSAPP_APP_SECRET` deployment variable.
5. Subscribe to the `messages` webhook field.

Every POST must include a valid `X-Hub-Signature-256` generated by Meta from the exact request body and App Secret. Missing or invalid signatures are rejected before parsing, database writes, or replies.
