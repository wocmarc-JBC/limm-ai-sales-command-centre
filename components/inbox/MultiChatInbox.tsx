...
import { isSilentCaptureMessage, silentCaptureSummary } from "@/lib/whatsapp-silent-capture";
import {
...
const loadInboxLeadContextPanel = () => import("@/components/inbox/InboxLeadContextPanel");
const InboxLeadContextPanel = dynamic(
  () => loadInboxLeadContextPanel().then((module) => module.InboxLeadContextPanel),
  {
    ssr: false,
    loading: () => (
      <aside className="flex h-full min-h-0 flex-col bg-command-panel2/95" role="status" aria-label="Loading conversation details">
        <div className="flex min-h-[4.5rem] items-center border-b border-command-line px-4">
          <div className="h-3 w-36 rounded-full bg-command-line/80 motion-safe:animate-pulse" />
        </div>
        <div className="grid gap-3 p-4">
          <div className="h-36 rounded-2xl border border-command-line bg-command-bg/55 motion-safe:animate-pulse" />
          <div className="h-24 rounded-2xl border border-command-gold/20 bg-command-gold/5 motion-safe:animate-pulse" />
          <div className="h-40 rounded-2xl border border-command-line bg-command-bg/55 motion-safe:animate-pulse" />
        </div>
      </aside>
    )
  }
...
const InboxSpamDialog = dynamic(
  () => import("@/components/inbox/InboxSpamDialog").then((module) => module.InboxSpamDialog),
  { ssr: false }
);
...
function bubbleTone(message: LeadMessage) {
  if (message.direction === "inbound") return "rounded-bl-md border-command-cyan/40 bg-[linear-gradient(145deg,#14202b,#101820)] text-[#fffaf0]";
  if (message.direction === "internal") return "border-command-line bg-command-bg/75 text-command-muted";
  if (message.metadata?.manualReply) return "rounded-br-md border-command-green/35 bg-[linear-gradient(145deg,rgba(35,170,115,0.14),rgba(16,29,26,0.92))] text-command-text";
  return "rounded-br-md border-command-cyan/35 bg-[linear-gradient(145deg,rgba(56,182,219,0.13),rgba(15,26,32,0.94))] text-command-text";
}
    ...
    <div
      ...
      className={`group relative border-b border-command-line/60 transition ${
        selected
          ? "bg-command-red/10"
          : active
            ? "bg-[linear-gradient(90deg,rgba(221,179,93,0.16),rgba(221,179,93,0.055)_64%,transparent)] shadow-[inset_3px_0_0_#DDB35D]"
    ...
    >
      ...
      >
        {selectionMode ? (
          ...
          <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-sm font-semibold text-command-gold ring-1 transition ${active ? "bg-command-gold/15 ring-command-gold/50 shadow-[0_8px_22px_rgba(221,179,93,0.12)]" : "bg-command-panel ring-command-line"}`} aria-hidden="true">
            {chatInitials(chat)}
          </span>
        )}
        ...
        {chat.unreadCount > 0 ? (
          <span className="mt-6 flex h-5 min-w-5 shrink-0 items-center justify-center rounded-full bg-command-gold px-1.5 text-[10px] font-bold text-black shadow-[0_0_0_3px_rgba(221,179,93,0.10)]">{chat.unreadCount}</span>
        ) : null}
      </a>
  ...
  return (
    <>
      <section className="inbox-product-frame flex flex-col overflow-hidden rounded-2xl border border-command-line bg-command-panel shadow-command">
        <div className={`${mobilePane === "chat" ? "hidden lg:flex" : "flex"} min-h-11 items-center justify-between gap-3 border-b border-command-line bg-[linear-gradient(90deg,rgba(221,179,93,0.08),rgba(18,23,30,0.94)_28%,rgba(18,23,30,0.90))] px-3 py-1.5 sm:px-4`}>
        ...
        <div data-testid="inbox-layout" data-mobile-pane={mobilePane} className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[18rem_minmax(0,1fr)] xl:grid-cols-[20rem_minmax(0,1fr)]">
          ...
          <main data-testid="inbox-active-chat" className={`${mobilePane === "chat" ? "flex" : "hidden"} min-h-0 flex-col bg-[radial-gradient(circle_at_top_left,rgba(221,179,93,0.045),transparent_34%),linear-gradient(180deg,rgba(9,14,20,0.99),rgba(5,7,10,0.99))] lg:flex`}>
            <header className="flex min-h-[3.75rem] shrink-0 items-center justify-between gap-2 border-b border-command-line bg-[linear-gradient(100deg,rgba(221,179,93,0.08),rgba(18,23,30,0.97)_32%,rgba(14,19,25,0.96))] px-2 py-1.5 shadow-[0_8px_26px_rgba(0,0,0,0.12)] sm:min-h-[4.25rem] sm:gap-3 sm:px-4 sm:py-2">
              <div className="flex min-w-0 items-center gap-3">
                <button
                  type="button"
                ...
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-command-gold/15 text-xs font-semibold text-command-gold ring-1 ring-command-gold/45 shadow-[0_8px_24px_rgba(221,179,93,0.12)] sm:h-10 sm:w-10 sm:text-sm" aria-hidden="true">
                  {chatInitials(chat)}
                </span>
                <div className="min-w-0">
                  <div className="flex min-w-0 items-center gap-2">
                    <h2 className="truncate text-base font-semibold text-command-text sm:text-lg">{chat.displayName}</h2>
                    <span className={`h-2 w-2 shrink-0 rounded-full ring-4 ring-white/[0.035] ${chatStatusDotTone(chat)}`} aria-hidden="true" />
                  </div>
              ...
              <div className="flex shrink-0 items-center gap-1 text-xs font-semibold">
                ...
                <button
                  ...
                  type="button"
                  onPointerEnter={() => void loadInboxLeadContextPanel()}
                  onFocus={() => void loadInboxLeadContextPanel()}
            ...
            <div ref={messagePaneRef} onScroll={handleMessagePaneScroll} data-testid="inbox-message-pane" className="thin-scrollbar min-h-0 flex-1 overflow-y-auto overscroll-contain bg-[radial-gradient(circle_at_18%_0%,rgba(56,182,219,0.035),transparent_30%),radial-gradient(circle_at_82%_100%,rgba(221,179,93,0.035),transparent_32%)] px-2.5 py-2 sm:px-5 sm:py-4">
              {awayFromLatest ? (
