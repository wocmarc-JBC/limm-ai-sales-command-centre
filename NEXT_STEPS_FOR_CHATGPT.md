...
1. Review `docs/V6_1_5_PERFORMANCE_FOLLOWUP_TEST_CLEANUP.md`.
2. Review `docs/V6_1_4_MISSION_CONTROL_UX_FINAL_POLISH.md`.
3. Review `docs/V6_1_2_MISSION_CONTROL_UI_LIVE_CLEANUP.md`.
4. Review `reports/V6_1_TEST_LEAD_CLEANUP_REPORT.md` after Marcus runs cleanup dry-run.
5. Confirm `/api/whatsapp/health` shows `version: v6_1_5_performance_followup_test_cleanup` before judging the deployed UI.
...
Prepare the controlled live v6.1.5 performance, follow-up button, and cleanup retest:
...
- Recommended next scope: confirm v6.1.5 health, review Settings cleanup counts for both leads and follow-ups, then use the in-app soft-delete/hide cleanup only if every identified record is clearly test data and Marcus/Fio/Fion are protected.
