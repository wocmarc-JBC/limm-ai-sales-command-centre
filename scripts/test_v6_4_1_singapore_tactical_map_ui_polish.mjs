...
assert(svgMapComponent.includes("singapore-silhouette-map"), "Singapore silhouette class/test id must exist.");
assert(svgMapComponent.includes("singapore-island-silhouette"), "Singapore island silhouette path must exist.");
assert(svgMapComponent.includes("viewBox=\"0 0 900 520\""), "Inline tactical Singapore silhouette SVG must be present.");
...
assert(!/fetch\(|googleapis|maps\.google|mapbox|geocode|GOOGLE_MAPS|MAPBOX/i.test(mapComponent + svgMapComponent), "No external geocoding or map API key should be added.");
