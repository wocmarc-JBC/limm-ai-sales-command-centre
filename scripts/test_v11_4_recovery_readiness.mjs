...
assert.match(database, /DATABASE_CORE_SCOPE_PROVIDER = "independent_pg_dump_s3_core_no_managed_schemas"/);
assert.match(database, /DATABASE_FULL_SCOPE_PROVIDER = "independent_pg_dump_s3_full_managed_schemas"/);
...
assert.match(evidenceRoute, /managedAuthIncluded/);
assert.match(evidenceRoute, /managedStorageIncluded/);
...
assert.match(workflow, /supabase db dump --db-url/);
assert.match(workflow, /database_user" != postgres\.\*/);
assert.match(workflow, /a scoped application role cannot read Supabase-managed Auth and Storage data/);
assert.match(workflow, /roles\.sql/);
assert.match(workflow, /schema\.sql/);
assert.match(workflow, /data\.sql/);
assert.match(workflow, /history_schema\.sql/);
assert.match(workflow, /history_data\.sql/);
assert.match(workflow, /aes-256-cbc -pbkdf2 -iter 200000/);
assert.match(workflow, /sha256sum.*--check --strict/);
assert.match(workflow, /postgres:17-alpine/);
assert.match(workflow, /limm-database-backup-v3/);
assert.match(workflow, /supabase_cli_portable_sql_bundle/);
assert.match(workflow, /independent_pg_dump_s3_full_managed_schemas/);
assert.match(workflow, /databaseScope:"full_database"/);
assert.match(workflow, /managedAuthIncluded:true/);
assert.match(workflow, /managedStorageIncluded:true/);
assert.match(workflow, /COPY "auth"\\\."users" .*encrypted_password/);
...
assert.match(workflow, /profiles_auth_user_fk/);
...
assert.match(workflow, /test "\$auth_users" = "\$DR_AUTH_USER_COUNT"/);
assert.match(workflow, /test "\$storage_objects" = "\$DR_STORAGE_OBJECT_COUNT"/);
assert.match(workflow, /isolatedRestore:true/);
assert.match(workflow, /workflow_restore_failed/);
assert.doesNotMatch(workflow, /create table auth\.users \(id uuid primary key\)/);
assert.doesNotMatch(workflow, /Stub auth rows contain IDs only/);
...
assert.match(operations, /Exact unresolved risk: Supabase-managed auth credentials/);
...
assert.match(health, /databaseLatestBackupScopeComplete/);
assert.match(health, /databaseCoreBusinessDataRecoveryProven/);
assert.match(health, /databaseRecoveryUnresolvedRisk/);
