...
const blockedPatterns = [
  ...
  { pattern: /\bguarantee(?:d)? approval\b|\bapproval (?:is )?(?:confirmed|guaranteed)\b|\bapproved\b/i, reason: "approval promise" },
  ...
  { pattern: /\bwe can definitely\b|\bguaranteed\b/i, reason: "over-promise" }
];
...
export function validateWhatsAppAutoReply(reply: string, options: { calendarEventId?: string } = {}) {
  ...
  if (calendarConfirmationPattern.test(reply) && !options.calendarEventId) {
    errors.push("final appointment confirmation without calendar event");
  }
