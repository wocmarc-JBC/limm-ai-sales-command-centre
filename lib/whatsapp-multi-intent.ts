...
const intentPatterns: Array<{ intent: WhatsAppDetectedIntent; pattern: RegExp; priority: number }> = [
  { intent: "complaint_or_risk", pattern: /\b(refund|lawyer|complaint|unhappy|defect|your work.*problem)\b/i, priority: 1 },
  { intent: "price_question", pattern: /\b(how much|roughly|price|cost|budget|estimate|quotation|quote|package)\b/i, priority: 2 },
  { intent: "appointment_request", pattern: /\b(appt|appointment|can make|book|schedule|wed(?:nesday)?|tomorrow|\d{1,2}\s*(?:am|pm)|site visit|come site|come down)\b/i, priority: 3 },
  { intent: "meeting_availability", pattern: /\b(next available|available slot|meeting|meet|when.*available)\b/i, priority: 4 },
  { intent: "hacking_wall", pattern: /\b(hack wall|can hack|remove wall|structural wall|load bearing|need pe|wall)\b/i, priority: 5 },
  { intent: "approval_submission", pattern: /\b(need approval|can approve|approval|permit|submission|ura|bca|will this pass)\b/i, priority: 6 },
  { intent: "portfolio_request", pattern: /\b(past works?|past projects?|project photos?|portfolio|before[-\s]?after|before and after|show me your work|photos of your works?|renovation photos?|completed project|design photos?)\b/i, priority: 7 },
  ...
  { intent: "greeting_ping", pattern: /\b(hello|hi|are you there|can reply|any update|\?)\b/i, priority: 12 },
  { intent: "generic_renovation", pattern: /\b(renovate|renovation|house|home|works?)\b/i, priority: 13 }
];
